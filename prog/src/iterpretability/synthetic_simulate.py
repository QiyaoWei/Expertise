...
class SyntheticSimulatorBase:
    ...
    def simulate_dataset(
        ...
        dist = torch.distributions.bernoulli.Bernoulli(torch.tensor(propensity))
        W_synth = dist.sample().long()
...
class SyntheticSimulatorLinear(SyntheticSimulatorBase):
    ...
    def __init__(
        ...
        self.prog_weights = np.random.uniform(-1, 1, size=(X.shape[1])) * self.prog_mask #np.ones(X.shape[1])
        self.pred0_weights = np.random.uniform(-1, 1, size=(X.shape[1])) * self.pred0_mask #np.ones(X.shape[1])
        self.pred1_weights = np.random.uniform(-1, 1, size=(X.shape[1])) * self.pred1_mask #np.ones(X.shape[1])
    ...
    def get_important_features(
        ...
        temp = 2
        if shift != 0:
            prog_indices[:temp*shift] = all_indices[2*num_important_features:2*num_important_features+temp*shift]
    ...
    def change_mask(self, shift) -> Tuple:
        self.prog_mask.fill(0)
        self.pred0_mask.fill(0)
        self.pred1_mask.fill(0)

        all_indices = self.all_indices
        num_important_features = self.num_important_features
        ...
        if shift != 0:
            ...
            pred0_indices[:shift] = all_indices[-shift-num_important_features:-num_important_features]
            pred1_indices[:shift] = all_indices[-shift-2*num_important_features:-2*num_important_features]

        self.prog_mask[prog_indices] = 1
        self.pred0_mask[pred0_indices] = 1
        self.pred1_mask[pred1_indices] = 1

    def reset_mask(self) -> Tuple:
        self.prog_mask, self.pred0_mask, self.pred1_mask = self.get_important_features(np.zeros((1, self.length)), self.num_important_features)

    def predict(self, X: np.ndarray) -> Tuple:
        # print(self.pred1_mask)
        prog = np.dot(X, self.prog_weights) # * self.prog_mask)
        pred0 = np.dot(X, self.pred0_weights) # * self.pred0_mask)
        pred1 = np.dot(X, self.pred1_weights) # * self.pred1_mask)
