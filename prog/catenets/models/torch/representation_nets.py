...
class BasicDragonNet(BaseCATEEstimator):
    ...
    def loss(
        ...
        # print(self.prop_loss_multiplier*prop_loss(t_pred, t_true))
        if self.ipw == 0:
            return (
            ...
            # likelihood = 1 / (1 + torch.exp(-(2 * t_true - 1) * t_pred))
            # treatment_probability = torch.mean(t_true.float())
            # w = 1 + treatment_probability / (1 - treatment_probability) * (1 - likelihood) /likelihood
            y0_pred = po_pred[:, 0]
            y1_pred = po_pred[:, 1]
            loss0 = torch.mean(torch.Tensor(p).to(DEVICE) * (1.0 - t_true) * head_loss(y0_pred, y_true))
            loss1 = torch.mean(torch.Tensor(p).to(DEVICE) * t_true * head_loss(y1_pred, y_true))
            return loss0 + loss1
    ...
    def train(
        ...
        if self.ipw == 1:
            _, p, _, _, p_val, _, _ = make_val_split(
                X, torch.Tensor(p).to(DEVICE), w=w, val_split_prop=self.val_split_prop, seed=self.seed
            )
        ...
        if self.ipw == 1:
            for param in self._propensity_estimator.parameters():
        ...
        for i in range(self.n_iter):
            ...
            for b in range(n_batches):
                ...
                if self.ipw == 1:
                    p_next = p[idx_next].squeeze()
                    # print(p_next.shape)
                    po_preds, prop_preds, discr = self._step(X_next, w_next)
                    batch_loss = self.loss(po_preds, prop_preds, y_next, w_next, discr, p_next)
                else:
                    po_preds, prop_preds, discr = self._step(X_next, w_next)
                ...
                # print(self._propensity_estimator.weight.grad.norm())
            ...
            if self.early_stopping or i % self.n_iter_print == 0:
                with torch.no_grad():
                    ...
                    if self.ipw == 1:
                        val_loss = self.loss(po_preds, prop_preds, y_val, w_val, discr, p_val)
                    else:
...
class CENet(BasicDragonNet):
    ...
    def _step(
        ...
        # print(self._propensity_estimator)
        prop_preds = self._propensity_estimator(repr_preds)
...
class IPWNet(BasicDragonNet):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        propensity_estimator = nn.Linear(n_units_r, 1).to(DEVICE)
    ...
    def pretrain(
        ...
        for i in range(self.n_iter):
            ...
            for b in range(n_batches):
                ...
                _, prop_preds, _ = self._step(X_next, w_next)

                likelihood = 1 / (1 + torch.exp(-(2 * w_next - 1) * prop_preds))
                ...
                optimizer.step()
        optimizer.zero_grad()
        self._propensity_estimator.eval()
